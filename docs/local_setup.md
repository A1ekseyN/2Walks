...
- **По mDNS hostname (рекомендуется):** <http://aleksey-H61M-DS2H.local:8008/> — резолвится автоматически на macOS / iOS через Bonjour, **переживает смену IP** (роутер при перезагрузке может выдать другой адрес).
- По IP: <http://192.168.1.155:8008/> — IP **меняется** при перезагрузке роутера (был `192.168.0.155` до 30.06.2026, стал `192.168.1.155`). Если перестал открываться — IP уплыл, узнай новый: `ping aleksey-H61M-DS2H.local`.

Bookmark mDNS-URL в Safari на iPhone + браузере MacBook'а — он не сломается при смене IP.

### Управление сервисом (на сервере через SSH)

```bash
ssh aleksey@aleksey-H61M-DS2H.local   # или по IP: ssh aleksey@192.168.1.155
...
```bash
make deploy                                # git pull + pip install + restart + is-active (HOST по умолчанию — mDNS)
make deploy HOST=aleksey@192.168.1.155     # принудительно по IP, если mDNS не резолвится
make status                                # systemctl status юнита
make logs                                  # journalctl -n 50 (FOLLOW=-f для live-хвоста)
```

`HOST` в `Makefile` по умолчанию — mDNS-имя `aleksey@aleksey-H61M-DS2H.local` (с 30.06.2026; устойчиво к смене IP — был `192.168.0.155`, стал `192.168.1.155`). `make deploy` разворачивается в ssh-команду (флаг `-t` обязателен — выделяет TTY, иначе `sudo` не сможет запросить пароль: «a terminal is required»):

```bash
ssh -t aleksey@aleksey-H61M-DS2H.local "cd ~/2Walks && git pull && .venv/bin/pip install -r requirements.txt && sudo systemctl restart 2walks && systemctl is-active 2walks"
```
...
```bash
alias 2walks-deploy='ssh -t aleksey@aleksey-H61M-DS2H.local "cd ~/2Walks && git pull && .venv/bin/pip install -r requirements.txt && sudo systemctl restart 2walks && sudo systemctl status 2walks"'
alias 2walks-logs='ssh aleksey@aleksey-H61M-DS2H.local "sudo journalctl -u 2walks -n 50 --no-pager"'
alias 2walks-status='ssh aleksey@aleksey-H61M-DS2H.local "sudo systemctl status 2walks --no-pager"'
```
...
|---|---|
| URL не открывается с MacBook/iPhone | Сначала проверь, не сменился ли IP сервера (после перезагрузки роутера): `ping aleksey-H61M-DS2H.local`. Затем: `ssh aleksey@aleksey-H61M-DS2H.local "sudo systemctl status 2walks"` — Active: failed? Лог `journalctl -u 2walks -n 50` покажет причину |
