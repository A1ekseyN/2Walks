...
**Total: 49 triumph'ов в 8 категориях, 7 seals** (с 0.2.6a +1 Wayfarer в Steps — реально пройденные `total_walked`, рядом с Marathoner, seal steps требует обоих capstone'ов; +1 Investor в Money 💰 — `cost_money` из `skill_train_start`, без seal) (с 0.2.5w +1 Iron Worker в Work; с 0.2.6 +1 Veteran в Progression ⭐ + seal Veteran; +6 Drops 💎 — Collector + 5 per-grade, seal Treasure Hunter). NB: тиры Veteran 15-30 пока недостижимы — level-кэп = 12, расширение в TASKS 4.64 (отложено). Drops: event-based на `[drop, drop_pending, drop_force_sold]` с grade-фильтром, tiers 10/50/100/250/500/1000.
