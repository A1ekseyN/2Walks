...
С 0.2.4f (4.29-replacement) каждый грейд в меню отображается с реальной вероятностью выпадения с учётом текущего luck: `(Награда: C-Grade [37.20%], B-Grade [33.36%])` (проценты в квадратных скобках). Расчёт — аналитический pure-helper `drop.compute_grade_probabilities(adv_name, state)` (без рандома). Item-type инфо («могут выпасть: ring · necklace · helmet · shoes · t-shirt + редкий backpack») вынесена во вступительный текст т.к. одинакова для всех приключений. С 0.2.6j (4.51) добавлен редкий тип **backpack** (надевается в слот `back`, +2/4/6/8/10 слотов инвентаря по грейду C..S+). **С 0.2.6l (4.65 — drop-modifier)** выбор ТИПА предмета в `item_type()` — взвешенная выборка `random.choices` (а не «5 кубиков max»): базовые веса (рюкзак 5, прочие по 19) смещаются к нужным игроку типам пропорционально `current_luck` — тип, где грейд игрока дальше от S+ (пустой слот / низкий грейд), получает буст; всё-S+ → базовое распределение. Кольца — по худшему из двух надетых пальцев. Grade-гейт «выпадет ли что-то» (`one_item_random_grade`) не меняется. Тиры (grade, threshold) хранятся в `adventure_data.py['drops']` — единый источник правды для аналитики и реальной `one_item_random_grade()`.
