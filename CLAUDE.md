...
9. **Gym training (0.2.1e + 0.2.1g).** `skill_training_check_done(state)` is called in `_dashboard_context` (alongside `work_check_done`) — auto-finalizes finished trainings. Start endpoint `POST /web/gym/start` (Form) / `POST /api/gym/start` (JSON `GymStartRequest`) validate via `_validate_and_apply_training(state, skill_name)` — pre-flight resource check, then existing CLI helper `Skill_Training.start_skill_training()` (try_spend + actions.start_training + decrease_durability), then persist. Pre-computed list of 8 skills via `_build_gym_skills(state)` returns `{cost, can_afford, missing}` for the UI; insufficient-resource buttons render disabled with explicit "Не хватает: 🏃 N" line. After 0.2.1g (4.48.4.1) all 8 skills work uniformly — `energy_max_skill` no longer special-cased.
