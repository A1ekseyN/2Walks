...
2. Google Sheets — three specialized worksheets in one spreadsheet (task 4.14 + 4.6, version 0.2.0d / 0.2.4):
    ...
    - `history` — append-only лог значимых игровых событий (task 4.6, since 0.2.4). Колонки: `ts | datetime | user_id | game_version | event_type | payload_json`. Dual-write: local `history.jsonl` (primary, sync, fail-resistant) + Sheets `history` (best-effort, fail-silent через `HistoryLogRepo.append`). Sheets копия для будущих dashboard'ов (4.10) и multi-device access. На каждое значимое событие (work_done, skill_upgraded, deposit, drop, level_up, item_bought / sold, etc. — 22 типа) в 15 mutation-сайтах вызывается `history.log_event(type, **payload)`. Schema event'а с версионированием: `{v, ts, date, time, user_id, game_version, type, payload}` — `v` для будущей миграции, `game_version` для отладки (понять какие фичи были на момент события). Pruning / CLI viewer / re-sync after Sheets recovery — отложенные follow-up подзадачи (4.6.1 / 4.6.2 / 4.6.3). Pytest fixture `tests/conftest.py` с autouse отключает `log_event` для всех тестов кроме `test_history.py` — иначе тесты загрязняют реальные local file и Sheets.
