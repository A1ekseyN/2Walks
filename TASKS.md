...
#### 4.54.0.2. Day rollover detection в web `[M / S / todo]`

**Контекст (02.05.2026, обнаружено при обсуждении 2.4):** в CLI day rollover (смена даты, перенос `today → yesterday`, обновление `daily_bonus`) детектится в `save_game_date_last_enter(state)` на каждом тике main loop. В web — `_dashboard_context()` НЕ вызывает `save_game_date_last_enter`. То есть если игрок открыл web в 23:59 и держит вкладку открытой до 00:30 — он не увидит rollover'а пока не сделает F5 / submit формы (тогда сработает `try_reload_state` → загрузит свежий state, но снова не вызовет `save_game_date_last_enter`).

**Реализация:**
- В `web/sync.py:try_reload_state()` после успешного reload — вызвать `save_game_date_last_enter(game.state)`. Это даст ту же точку проверки rollover, что в CLI.
- Опционально: при `GET /` (после reload) — тоже вызвать. Дублирование с try_reload_state не страшно (idempotent — на тот же день no-op).
- Тест: `state.date_last_enter = '2020-01-01'` (чтобы триггернуть rollover) → `try_reload_state()` → проверить `state.steps.today == 0` и т.д.

**Effort:** S (2-3 строки кода + тесты).

**Зависимость:** не блокирующая. Сейчас не критично — игрок обычно делает F5 утром следующего дня, что триггернёт reload + max-merge через 4.15 (steps_today подтянется правильно). Но `daily_bonus` без явного rollover не обновится.
