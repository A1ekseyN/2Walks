...
### 1.3. Разделить `functions.py` и `characteristics.py` по смыслу (зонтичная) `[L / M / partial (21.05.2026 — 1.3.4 rejected; 4 из 7 подзадач сделано: 1.3.1/2/2.1/3 done; 1.3.5 отложен; 1.3.6 web/ split добавлен как новая L задача)]`
...
- ✅ 1.3.3 (save/load → persistence.py, 0.2.4y / 1.4.2 combined).
- ❌ 1.3.4 — **rejected** (21.05.2026 — пользы маргинально, нет реальных pain points, CLI-only функции).
- 🟡 1.3.5 — отложено (full package layout, нет critical pain points).
- 🆕 1.3.6 — **новая задача (21.05.2026)** разделить `web/main.py` (3071 строк) на routes/ + views/. Реальный pain point: web/main.py в 4× больше следующего файла.

Зонтичная задача считается **80%-выполненной по infrastructure**. Остаток — focused web/ split (1.3.6) + full package layout (1.3.5, deferred).
...
#### 1.3.4. Вынести UI helpers (status_bar, char_info, format_steps) `[L / S / rejected (21.05.2026 — обсудили, не нужно)]`

**Решение (21.05.2026):** **Rejected.** Тех-долг без реального impact'а.

**Обоснование отказа:**

1. **functions.py = 327 строк (07.05.2026 было 298)** — за 2 недели прирост всего +29 строк. **Под threshold'ом 500** для refactor'а.
2. **0 реальных pain points** — навигация работает через grep / IDE, циклических импортов нет, тестам всё равно где функции лежат.
3. **Реально UI-only — только 3 функции** (`status_bar`, `char_info`, `location_change_map`) на ~110 строк. Минимальный scope.
4. **CLI-only функции** (status_bar и т.д. печатают через `print`) — не переиспользуются web'ом (web использует Jinja templates). Отделять UI слой искусственно — нет второго consumer'а.
5. **Risk циклических импортов** — `status_bar` тянет CharLevel / equipment_bonus / skill_bonus / format_timedelta / compute_energy_max. После split может вылезти cycle.
6. **Pattern из сегодняшней grooming-сессии** — мы reject'ули 4 похожих refactor-only tasks (1.6 items dataclass, 4.24 split luck, 4.25 grade upgrade) с тем же reasoning: нет user-visible benefit'а + risk без gain'а.
7. **Реальная structural боль не здесь, а в `web/main.py` (3071 строк)** — выделена в новую задачу 1.3.6.

**Когда переоткрыть:**
- Если functions.py перевалит 500 строк.
- Если появится **реальная** боль навигации (потерянные функции, dependency loop'ы).
- Если когда-нибудь будет 2-й UI (TUI, Discord bot) который захочет переиспользовать status_bar logic.

**Что осталось как-есть:** functions.py 327 строк (15 функций — state logic + pure calcs + 3 UI helpers). Pattern: helpers по месту, не выделение «UI layer» искусственно.

#### 1.3.5. Полный package layout (game/persistence/ui/tables/api/) `[L / L / todo (отложено — CLI модули workable, нет critical pain points)]`

Архитектурный рефакторинг всего проекта с разделением на пакеты (`src/2walks/<category>/`). **Не делаем сейчас** — overhead > польза.

**Обновлённый статус (21.05.2026):** проект вырос до ~11,700 LoC (8500 CLI + 3200 web), но **все CLI модули workable** (200-700 LoC, comfortable single-purpose размер). Единственная реальная structural боль — `web/main.py` (3071 строк) — выделена в **focused задачу 1.3.6** (web/ split only). Full package layout остаётся deferred — нужен только если CLI модули начнут разрастаться или появится несколько контрибуторов.

#### 1.3.6. Разделить web/main.py на routes/ + views/ `[L / M / todo (21.05.2026 — focused web/ split)]`

**Контекст:** `web/main.py` сейчас **3071 строка** — в 4× больше следующего по размеру файла (forge.py 720). Содержит ВСЁ web-логику в одном файле: ~30 FastAPI endpoints + ~10 view builders + ~10 validate_and_apply helpers + ~15 Pydantic models + STALE handling + app init. Это классический FastAPI anti-pattern — лучшая практика разбивать по resource.

**Pain points** при текущей структуре:
- Найти Bank endpoint = scroll через 1500+ строк UI helpers
- Добавить новый endpoint = найти правильное место среди 3K строк
- Onboarding contributor'а = читать весь файл прежде чем понять architecture
- Когда буду делать 4.48.7 (Web Shop) — добавится ещё ~300 строк → 3400 LoC

**Target структура (Variant 1 per design discussion 21.05.2026):**

```
web/
├── __init__.py
├── main.py              # ~100 строк — только app init + lifespan + mount + register routers
├── sync.py              # ✓ already exists (try_reload_state / persist_state_to_cloud)
├── stale.py             # NEW — STALE handling (_stale_response, _stale_json_response,
│                        #   _persist_and_handle_stale, _render_dashboard_or_stale)
├── models.py            # NEW — все Pydantic Request classes (WorkStartRequest,
│                        #   InventorySellRequest, EquipmentWearRequest, BankAmountRequest, ...)
├── context.py           # NEW — _dashboard_context orchestrator (~200 строк)
├── views/               # NEW — pre-computed view builders для template'ов
│   ├── __init__.py
│   ├── inventory.py     # _build_inventory_view, _sort_inventory_view
│   ├── equipment.py     # _build_equipment_view
│   ├── adventure.py     # _build_adventure_view, _finalize_adventure_with_drop_capture
│   ├── bank.py          # _build_bank_view, _validate_and_apply_bank_op
│   ├── work.py          # _build_work_*, _max_work_hours, _build_hour_options,
│   │                    #   _validate_and_apply_work_start/_add_hours
│   ├── gym.py           # _build_gym_skills, _validate_and_apply_training
│   ├── loadout.py       # _build_loadout_view, _build_optimize_preview,
│   │                    #   _build_preset_preview, _validate_and_apply_{optimize,save_preset,...}
│   ├── report.py        # _build_away_report_view
│   └── stats.py         # _build_low_quality_warning, прочее для Stats area
├── routes/              # NEW — endpoint groupings по resource
│   ├── __init__.py      # register все APIRouter'ы в одном месте для main.py
│   ├── dashboard.py     # GET / + GET /status + GET /healthz
│   ├── steps.py         # POST /web/steps, POST /api/steps
│   ├── work.py          # POST /web/work/start, /add_hours + /api/work/*
│   ├── gym.py           # POST /web/gym/start, /api/gym/start, /web/level/allocate, /api/level/allocate
│   ├── inventory.py     # POST /web/inventory/sell, /api/inventory/sell
│   ├── equipment.py     # POST /web/equipment/wear, /unwear + /api equivalents
│   ├── bank.py          # POST /web/bank/{deposit, withdraw, take_loan, repay_loan, ...} + /api/bank/*
│   ├── adventure.py     # POST /web/adventure/start, /api/adventure/start
│   ├── loadout.py       # POST /web/loadout/{preview, optimize}, /web/preset/{save, load, delete, preview_load}
│   └── drop.py          # POST /web/drop/{sell_existing, sell_new, skip}, /api/drop/*
├── templates/           # ✓ already exists
└── static/              # ✓ already exists
```

**После split:**
- `web/main.py` → ~100 строк (только app init, регистрация routers'ов)
- Каждый concern в своём файле (200-400 строк) — workable size
- **Найти любой endpoint** → 1 hop через директорию `routes/`
- **Понять view** → одно место `views/<resource>.py`
- **Best practice FastAPI** — separate APIRouter per resource

**Effort:** **M** (~4-6 часов реального времени)

Шаги:
1. Создать `web/routes/`, `web/views/` директории + `__init__.py`
2. Извлечь `stale.py` (4 функции, ~80 строк) — простой move
3. Извлечь `models.py` (15 Pydantic классов, ~150 строк)
4. Извлечь `views/<resource>.py` по одному resource (test after each)
5. Извлечь `routes/<resource>.py` с APIRouter pattern — register в `routes/__init__.py`
6. `context.py` — _dashboard_context + _render_dashboard_or_stale
7. `main.py` рекомендую переписать с нуля (only ~100 lines)
8. Update imports в tests (`from web.main import X` → `from web.views.X import Y` etc.)

**Risks:**
- Circular imports — `routes/` импортит `views/` импортит `state` / `persistence`. Tests + careful import ordering.
- Tests — 1095 passed. После split нужно фиксить ~30-50 test imports.
- Live deploy — нужно проверить что Sheets read/write работает после refactor (нет hidden state side effects).

**Тесты:** все existing 1095 должны продолжать проходить (test logic не меняется, только imports updated). Опционально добавить 1-2 test'а для проверки router registration.

**Зависимости:** не блокирует ничего, не блокируется ничем. Но:
- Делать **до** 4.48.7 (Web Shop) — если Shop добавит endpoints в текущий monolithic main.py, разгребать сложнее.
- Делать **после** реализации крупных new web фич (если планируются) — refactor одновременно с feature work рискован.

**Приоритет:** **L** — реальный pain, но не блокирует gameplay. Optional improvement когда будет время + желание.
...
### 1.6. Items как dataclass, не как dict со списками-обёртками `[M / M / rejected (21.05.2026 — обсудили, не нужно)]`

**Решение (21.05.2026):** **Rejected.** Оставляем текущий dict-with-list-wrappers формат.

**Обоснование отказа:**

1. **0 реальных bugs за 2 месяца.** С момента «отложено» (08.05.2026) не было ни одного incident'а связанного с забытой `[0]` или опечаткой в item-key. Текущий формат стабильно работает в production.
2. **Большой refactor scope.** 62 ссылки `item['key']` в 6 файлах + ~30 тестов + state serialization + save format migration + web template fragments. **300+ строк изменений** в 9-10 файлах — один из самых больших refactor'ов в проекте.
3. **Player не увидит ни одного visible улучшения** — pure tech-debt.
4. **Save format migration на live production** — серьёзный risk. Урок 4.48.5.4 (UNFORMATTED_VALUE сюрприз) показал что даже маленькие изменения format'а могут скрыто сломать.
5. **Helpers pattern работает лучше.** За последние месяцы появились точечные helper'ы которые решают «awkward access» по месту: `equipment_bonus._is_broken(item)` (4.61), `shop._first(values)` (Shop), `report._extract_field(item, key)` (4.2). Composition over rewriting — более pythonic.
6. **Original blocker отпал.** В записи 08.05.2026 рекомендовалось «делать вместе с 4.7 Shop». 4.7 теперь L (нужен design прежде кодить) — обоснование «делать вместе» больше не актуально.
7. **TypedDict альтернатива тоже отклонена.** Обсудили: TypedDict дал бы catch опечаток + IDE help (~30 мин), но и тут реальных bugs нет → «решение в поисках проблемы».

**Когда переоткрыть (re-open triggers):**
- Появится **реальный** bug из-за забытой `[0]` или опечатки в item-key — тогда взвесить TypedDict vs полный refactor.
- Понадобятся items с **разными полями** (мульти-effect items, бустеры с durations и т.п.) где dict-with-lists станет mismatch'ем для structure'ы.
- Crucial: если refactor станет естественной частью большой фичи (например полная переработка drop/inventory), включить как часть scope'a.

**Что осталось как-есть:** item-dict с list-обёртками (`item['grade'][0]`, `item['bonus'][0]`) во всех 6 production-файлах. Type hints на parameters — plain `dict` (как в комментарии 5.1 type hints coverage в CLAUDE.md). Точечные helper'ы добавляются по необходимости.
...
### 4.9. Мульти-сейв `[L / M / todo (M → L 21.05.2026)]`
...
**Идея:** если игрок несколько раз подряд прошёл walk и ничего не выпало — шанс получить предмет растёт. При успешном дропе счётчик сбрасывается.

> 📄 **Полный дизайн + статистика (вероятности дропа по всем walk × luck, MC-симуляции) — [`docs/drop_mechanics.md`](../docs/drop_mechanics.md) §7.** Ниже — краткая выжимка.

**Финализированный дизайн (17.06.2026): RE-ROLL (вариант 2), без cap.**

- Счётчики **per-walk** (7 штук): `c` = число приключений **подряд**, завершившихся полностью пустыми.
- На заходе делаем **`1 + c` независимых вызовов** `one_item_random_grade`, останавливаемся на первом дропе.
- Заход полностью пустой (все попытки miss) → `c += 1`. Любой дроп → `c = 0`.

**Почему именно re-roll, а не «+5% к шансу за промах» (отвергнуто):**
- Дроп двухстадийный (гейт + конкуренция тиеров грейда, см. docs §2). Буст гейта упирается в «пол» вероятности из-за 2-й стадии — на endgame почти не работает (walk_30k 34→37% максимум, серии до 27 остаются). Saturation на pity≈2–4.
- Re-roll = свежий двухстадийный ролл → **пробивает пол** (`P = 1−(1−p)^(1+c) → 1`) и **сохраняет grade-иерархию** (walk_30k всегда только S+, walk_easy от re-roll'ов S+ не родит).

**Подтверждено симулятором** (`scratch_pity_sim.py`, MC 200k приключений/клетку, luck 1..30):
- Worst-case серия пустых заходов подрезана с 27–31 → **5–7** даже при luck=1, встречается ~1–10 раз на 200k (≈0.0005–0.005%).
- Стоимость механики — ср. **1.0–1.55** попытки на поход (включается редко, только при невезении).
- DROP-RATE endgame (luck=10): walk_30k 34→49%, walk_25k 42→55%; иерархия walk сохранена.
...
### 4.25. Новый навык: Grade Upgrade Chance (бонус к грейду дропа) `[M / S / rejected (21.05.2026 — обсудили, не нужно)]`

**Решение (21.05.2026):** **Rejected.** Оставляем текущую систему walk-tier drops без post-roll upgrade chance.

**Обоснование отказа:**

1. **Размывает walk progression — core gameplay loop.** Walks намеренно тиерированы: walk_easy (только C, entry) → walk_30k (endgame S+, ~35% chance после 0.2.4g balance). Если skill позволяет получить S+ из walk_easy, **зачем идти на walk_30k**? Подрывает основную мотивацию прогрессии.
2. **Conceptual overlap с Luck — тот же урок что reject 4.24.** Игрок спросит «в чём разница между Luck и Grade Upgrade?». Оба дают «лучше дропы». Reject 4.24 (Split Luck) был ровно по этой причине — multiple loot stats путают (урок Diablo 3 убирания MF Quantity).
3. **Балансировка тяжёлая, no good answer.** Линейный +1%/lvl на lvl 50 = 50% upgrade → broken. Cap (30-40%) — лучше, но S+ из walk_easy перестаёт быть jackpot'ом. Diminishing returns — добавляет complexity.
4. **Текущая система работает.** walk_25k ~20% S+, walk_30k ~35% S+ — это уже high chances. Нет «не могу получить S+» проблемы.
5. **Нестандартный паттерн в RPG.** Diablo 3 / PoE / Borderlands / WoW — все используют zone-based drop tables или rarity multipliers. **Нет** примеров где skill «promote dropped item one tier up». В Slay the Spire это active mechanic (cards/relics), не passive skill.
6. **«Решение в поисках проблемы».** Нет user-facing frustration которую 4.25 решает. Есть теоретическая «больше hype» — но это можно дать через другие механики.

**Альтернативы (уже в TASKS, более targeted):**

- **4.19 Pity system** (M / M / todo) — гарантированный rare drop после N неудачных attempt'ов. Решает frustration более точно без размывания walk hierarchy.
- **4.45 Weekend Bonus / Quests** (M / S / todo) — temporal hype через +X% drop rates по выходным или quest rewards.

**Когда переоткрыть:**
- Если появится **конкретная** user frustration о «застрял на C-grade из walk_easy, хочу хайп без апгрейда walks».
- Если 4.19 Pity system после реализации окажется недостаточной для hype-моментов.
- Если walk progression перестанет быть meaningful (например, все walks unlocked и игрок не ходит на низкие) — тогда upgrade chance может стать способом дать ценность low-walks. Сейчас walks тиерированы и progression имеет смысл.

**Что осталось как-есть:** текущая walk-tier system с luck_skill влияющим на gate / grade / quality. Никаких новых post-roll upgrade chances.
...
### 4.34. Прогресс-бар разблокировки приключений (вместо порога) `[L / S / done (26.05.2026, 0.2.6)]`

Сейчас walk_15k разблокируется после 3 прохождений walk_hard — резкий threshold. **Идея:** показывать прогресс к разблокировке.

**Реализовано (26.05.2026, 0.2.6):** глиф-бар прогресса (как в Triumphs, `▰▱`) вместо «осталось N раз».
- **Формат (выбран пользователем):** глиф-бар везде (CLI + web), переиспользован `triumphs._format_progress_bar(width=3)`. CLI: `🔒 Прогулка по району: ▰▰▱ 2/3 (67%) — пройдите «Прогулка вокруг озера»`; web — тот же бар + текст в Adventure-секции (`adv.unlock_bar` + `adv.unlock_hint`).
- **Охват:** бар только у **первой запертой** прогулки (реально прокачиваемой); более глубокие — «🔒 заблокировано» (цепочка из 0% шумна).
- **Рефактор (выбран пользователем):** порог `3` + цепочка вынесены в `adventure_data.py` — `ADVENTURE_UNLOCK_THRESHOLD` / `ADVENTURE_PREREQ` / `ADVENTURE_RU_LABELS`, единый источник для CLI и web. Раньше дублировались: литерал `3` в 7 местах `adventure.py` + dict `_ADVENTURE_UNLOCK` в `web/main.py` (удалён). `_render_adventure_menu` переписан с 7 if/else на data-driven loop.
- **UI-only** — логика разблокировки не менялась. NB: CLI-выбор пункта меню исторически не enforce'ит locked (косметика), web — enforce через `_validate_and_apply_adventure`.

**Файлы:** `adventure_data.py` (константы), `adventure.py` (loop + бар), `web/main.py` (`_build_adventure_view` + `_validate_and_apply_adventure`, удалён `_ADVENTURE_UNLOCK`), `_status_fragment.html` (рендер бара). **Тесты:** 1 обновлён + 1 новый. 1303 passed, mypy 0 issues.
...
#### 4.48.6.1. Web: «Надеть»/«Заменить» прямо из секции Экипировки `[S / S / done (17.07.2026, 0.2.7g)]`

**Проблема (UX-дыра, найдена игроком 16.07.2026):** секция Экипировки умела только «Снять» — пустой слот показывал «— пусто —» без действий, надевание жило исключительно в секции Инвентаря. Сценарий-ловушка: снял предмет → слот пуст → тупик → игрок идёт в другое окно искать «как надеть» → реальный инцидент закончился **случайной продажей** ожерелья S-Grade +4 energy_max (восстановлено вручную по Sheets history: качество/цена вычислены точным обратным расчётом refund 67.84 ÷ 1.06 Торговец = 64 $ → quality 32.0; audit-событие `item_restored`).

**Реализовано (17.07.2026, 0.2.7g) — вариант A (inline select) + swap в занятых (выбран из 3 вариантов: A inline / B two-step ➕ / C ссылка в Инвентарь):**
- Каждый слот с подходящими предметами в инвентаре рендерит компактный `<select>` кандидатов + кнопку: пустой слот — «🧥 Надеть», занятый — «🔄» (замена через существующий auto-swap `_equip_from_inventory`, net-zero по вместимости — работает и при полном рюкзаке).
- Кандидаты отфильтрованы по типу слота (реверс-карта `_SLOT_TO_ITEM_TYPES` из `_ITEM_TYPE_TO_SLOTS`; кольца → оба пальца; слот «Ноги» — типа нет в игре, всегда «— пусто —»), сортировка bonus desc → quality desc, label через `_equipment_candidate_label` («Necklace S +4 Energy_Max · Q 32.0»; рюкзак «+N слотов»; broken префикс 🔨).
- Ноль новых endpoints — формы постят в существующий `/web/equipment/wear` (inventory_index + slot_attr + sort). Поле `candidates` добавлено в `_build_equipment_view`.
- «— пусто —» остаётся только у слотов без кандидатов.
- 8 новых тестов в `tests/test_web_main.py` (фильтрация, сортировка, label формат + backpack/broken, рендер пустого/занятого/без кандидатов, полный endpoint flow). 1544 passed, mypy 0 issues.

#### 4.48.7. Web: Shop `[L / M / todo (21.05.2026 — M → L; blocked by 4.7 которая также L)]`
...
##### 4.48.12.1. Web: итог дропа в баннере «Приключение пройдено» `[M / S / done (15.06.2026, 0.2.6n)]`

**✅ Реализовано (15.06.2026, 0.2.6n).** Закрывает «открытый под-вопрос» из п.2 выше (объединять ли «🗺 Приключение пройдено» + «🎁 выпало X»). **Решение (AskUserQuestion 15.06.2026): объединить.** Раньше было два баннера — «Приключение пройдено» (только имя прогулки) и отдельный зелёный `adventure-drop-notification` (item + кнопки, ТОЛЬКО при дропе); при отсутствии находки результат не показывался вовсе. Теперь один баннер: имя прогулки + строка `🎁 Нашли: Ring (B) · +5 Luck · качество 87.0 · 120 $` (с кнопками **🧥 Надеть / 💰 Продать**, кольцо → «На палец 1/2») либо `🔍 Находок нет`.

**Механика:** финализатор `_finalize_adventure_with_drop_capture` извлекает итог из буфера `deferred` (`drop` / `drop_pending` / `drop_force_sold` от `item_collect`) → статичный снапшот находки + `drop_ref` (object-ref на dict в инвентаре, безопасно т.к. `session_events` runtime-only) в payload `push_session_event('adventure_done', ...)`. Кнопки в `_build_session_events_view` резолвят индекс по identity (`is`) per-event (а не по глобальному `last_adventure_drop` — корректно при нескольких adventure_done подряд) — пока находка в инвентаре; после mutation предмет уходит → кнопки исчезают, текст «Нашли…» остаётся как лог. Грейд через `grade_short` («S+»). Удалены: зелёная секция `adventure-drop-notification`, helper `_build_drop_notification_view`, context-ключи `drop_notification` / `drop_notification_view`. Тип предмета — английский `|title` (русификация типов = отдельная сквозная задача). Файлы: `web/main.py`, `_status_fragment.html`.

**Тесты:** 4 теста бывшего зелёного баннера переписаны на новую модель (через `adventure_done` session-event, scoped к секции `session-events`) + новый тест «Находок нет». 1503 passed, mypy 0 issues.
...
#### 4.49.1. Skill `banking_interest_rate` (зонтичная) `[M / S / done (0.2.2 + 0.2.4z — все подзадачи 4.49.1.0/1.1/1.2 done)]`
...
#### 4.49.2. Кредиты (зонтичная) `[M / M / done (0.2.2 + 0.2.4z — 4.49.2.0/2.1/2.3 done; 4.49.2.2 auto-repay вынесен как deferred follow-up)]`
...
### 4.51. Рюкзак — equipment-предмет на вместимость (drop) `[M / M / done (29.05.2026, 0.2.6j)]`

**✅ Реализовано (29.05.2026, 0.2.6j) — V1 (только вместимость).** item_type `backpack` + слот `back` («Спина»), дроп флэт 5% (`BACKPACK_DROP_CHANCE`, пре-гейт в `item_type`), грейды C/B/A/S/S+ → +2/4/6/8/10 слотов (`BACKPACK_GRADE_SLOTS` от грейда). `bonus.backpack_capacity` = base + skill + `backpack_slots_bonus` (gated by `_is_broken`). Сломан → 0. characteristic `backpack_capacity` (не optimizable → loadout не трогает). Износ/ремонт/craft — как все. Display «+N слотов» (CLI слот 7 + opt/preset 8/9; web). Слоты добавлены в: state.Equipment+round-trip, equipment_bonus, inventory durability, forge, equipment.py, sync_diff, web. **Тесты:** +17 (`test_backpack.py` 14 + web 3). 1462 passed, mypy 0. Усиление — 4.65 (drop-modifier) + 4.66 (мульти-параметр).



**Концепция переосмыслена (29.05.2026):** рюкзак — НЕ квестовый flag, а **обычный equipment-предмет**, который **дропается в Adventure** (редко) и занимает **новый слот «Спина»**. Даёт **+N слотов инвентаря** в зависимости от грейда. Имеет quality / износ / ремонт (как вся экипировка). Дополняет навык 4.50 (`backpack_skill`) аддитивно.

**Финализированный дизайн (V1 — только вместимость):**

| Решение | Выбор |
|---|---|
| Тип / слот | Новый `item_type = 'backpack'`, новый equipment-слот **`back`** («Спина»). Чистый новый слот — `legs` не трогаем (зарезервирован под будущее). |
| Грейды → слоты | C +2 · B +4 · A +6 · S +8 · S+ +10. **Берём от ГРЕЙДА** (`BACKPACK_GRADE_SLOTS` lookup), НЕ от поля `bonus` — чтобы craft/drop апгрейд грейда проставляли capacity сами. |
| characteristic | `'backpack_capacity'` (только вместимость, V1). НЕ входит в `OPTIMIZABLE_CHARACTERISTICS` → loadout-оптимизатор слот не трогает (нет footgun'а «потерять capacity ради статы»). Stat-рюкзак — позже через 4.66 (мульти-параметр). |
| capacity-формула | `bonus.backpack_capacity(state) = BASE(10) + skill + (BACKPACK_GRADE_SLOTS[worn.grade] если рюкзак надет И не сломан)`. Единственная точка — разойдётся по всей игре (inventory_full / shop / drop / UI). |
| Сломан (quality 0) | **0 слотов** (как все broken, механика 4.61 → стимул чинить). Инвентарь может стать over-capacity — без краша (`inventory_full` использует `>=`, нельзя класть новое пока не починишь). |
| Дроп | Флэт **5%** среди *выпавших* предметов. Двухстадийно в `drop.item_type()`: пре-гейт `if random() < BACKPACK_DROP_CHANCE(0.05): return 'backpack'`, иначе существующий max-ролл из 5 типов. **Grade-гейт (выпадет ли что-то) НЕ трогаем** → общий шанс дропа не меняется; рюкзак рідкий относительно других типов. tunable-константа. |
| Quality / ремонт | Как все: дроп даёт quality `randint(20+luck,100)`, износ через `decrease_durability`, ремонт через forge Repair (подхватит сам). |
| Crafting | Разрешён — 2 одинаковых C-рюкзака → 1 B (работает сам через forge: все рюкзаки одного грейда идентичны по `item_type+characteristic+grade`). Capacity апгрейдится с грейдом. |
| Display | «+N слотов» (спец-формат для `item_type=='backpack'` в инвентаре/экипировке), не generic `bonus`. |

**Реализация (места):**
1. `state.Equipment` — новый слот `back` + round-trip (`equipment_back`) + `update_from_dict`.
2. `drop.py` — `item_type()` пре-гейт 5%; `item_collect` ветка для backpack (characteristic='backpack_capacity', bonus/grade); `BACKPACK_DROP_CHANCE` + `BACKPACK_GRADE_SLOTS`.
3. `bonus.backpack_capacity` — + вклад надетого рюкзака (от грейда, gated by `_is_broken`).
4. Перечисление слотов: `forge._EQUIPMENT_SLOTS`, web `_EQUIPMENT_SLOT_LABELS` + `_ITEM_TYPE_TO_SLOTS` (`backpack→['back']`), loadout (slot list), equipment_bonus (slot scan — backpack characteristic не стата, в агрегатор не попадёт).
5. UI: CLI Equipment menu + web Экипировка/Инвентарь — display «+N слотов» + надеть/снять в слот «Спина».
6. Тесты: round-trip слота; capacity от грейда; broken→0; drop 5% (mock random); craft 2C→1B; UI.

**Зависимость:** **4.50** (базовая capacity-механика — done). Усиливается опциональными **4.65** (drop-modifier: нет рюкзака → +luck к шансу) и **4.66** (мульти-параметр: рюкзак + stata). Обе — отдельные задачи, не блокируют V1.
...
#### 4.54.0.3. Smoke-test isolation: dry-run или test user_id для live API проверок `[M / S / done (29.05.2026, 0.2.6k — Вариант A: DRY_RUN)]`

**✅ Реализовано (29.05.2026, 0.2.6k) — Вариант A (dry-run).** Флаг `settings.dry_run` (env `DRY_RUN=1`). Гейт на persistence-слое (защищает web-смок И `python -c`-смок): no-op в `GameStateRepo.save`/`save_safe`, `StepsLogRepo.append`, `HistoryLogRepo.append`, `persistence.save_characteristic` (early "OK", skip state.json), `history.log_event` (skip jsonl/Sheets/register_event). Reads работают → смокаем поверх RAM-копии реального state. Предупреждение на старте в `init_game_state`. Доку — в `docs/local_setup.md`. Вариант B (отдельный smoke-spreadsheet для реального round-trip) — отложен до появления CI. **Тесты:** +6. 1468 passed, mypy 0.
...
#### 4.54.0.4. Fix: rollover persist в init (двойной «New Day» web↔cli) `[M / S / done (27.05.2026, 0.2.6)]`

**Баг (замечен Oleksii 27.05.2026):** играешь в web (или cli), потом запускаешь другой процесс (cli/web) в тот же новый день — и снова видишь «New Day: ...». Причина: CLI rollover делался на первом тике main loop (`functions.status_bar → save_game_date_last_enter`) **без немедленного persist**. На старте порядок был: skill-finalize save (persist с ещё **вчерашней** датой) → потом rollover в status_bar (меняет дату только в RAM). Sheets оставался со вчерашней датой до следующего user-save → другой процесс при load перечитывал yesterday и re-fire'ил rollover.

**Fix (proposal A):** rollover перенесён в `characteristics.init_game_state` (после `apply_steps_log_max_merge`, до triumph-блока) для ОБЕИХ платформ — симметрично web `try_reload_state`. При **реальной** смене дня (непустой `_old_date != today`) сразу вызывается `save_characteristic()` → дата фиксируется в Sheets немедленно, повтор не возникает. Пустая дата (новая игра / fresh state) НЕ триггерит persist (нет предыдущего дня — не сценарий double-rollover; защищает тесты на `default_new_game` + e2e reload). `status_bar` / `_dashboard_context` продолжают звать `save_game_date_last_enter` как defense-in-depth (same-day no-op).

**Файлы:** `characteristics.py` (init rollover + conditional persist). **Тесты:** 2 новых (`test_init_persists_rollover_on_real_day_change`, `test_init_no_persist_same_day`). 1317 passed, mypy 0 issues.

**Не закрыто (отдельно):** batch-чтение 3 листов одним `values_batch_get` (proposal C) — оптимизация латентности старта, см. ниже 4.54.0.5.

#### 4.54.0.5. Startup latency: batch-чтение Sheets (game_state + steps_log + history) `[L / M / todo (27.05.2026)]`

**Контекст:** на старте init делает 3 отдельных Sheets-READ (game_state load + steps_log max-merge + history away-report) + ещё reads/writes на первом тике (finalizers). Идея — объединить 3 init-чтения в один `spreadsheet.values_batch_get(['game_state!A1:B1', 'steps_log!A:D', 'history!A:F'])`, пропарсить секции, скормить существующим парсерам.

**Реалистичная оценка выгоды:** видимые «2.63 сек» Data Loaded — это в основном one-time gspread client auth + первый round-trip, НЕ 3× чтения. Батч сэкономит ~0.6–1 сек (2-й и 3-й round-trip'ы). Умеренно.

**Риск:** затрагивает repo-слой (`google_sheets_db` layout auto-detect JSON-blob vs legacy Key/Value, `_ensure_sheet` auto-create, CSV/local fallback) + persistence + characteristics + report. Нужен аккуратный fallback на текущий per-call путь при любой ошибке batch'а (worst case — не медленнее сегодняшнего). Делать focused-задачей.

#### 4.54.1. Schema + last_modified + snapshot mechanism `[H / S / done (0.2.4p, 15.05.2026 — реализовано в рамках 4.54)]`
...
#### 4.54.2. Core repo helpers: load_meta + save_safe `[H / S / done (0.2.4p, 15.05.2026)]`
...
#### 4.54.3. diff_states helper + format functions `[H / M / done (0.2.4p, 15.05.2026 — sync_diff.py)]`
...
#### 4.54.4. save_characteristic + persist_state_to_cloud wrappers `[H / S / done (0.2.4p, 15.05.2026)]`
...
#### 4.54.5. CLI STALE prompt `[H / M / done (0.2.4p, 15.05.2026 — handle_stale_prompt)]`
...
#### 4.54.6. Web UI STALE flash + auto-reload `[H / M / done (0.2.4p, 15.05.2026)]`
...
### 4.55. Auth для web `[L / S / todo (M → L 21.05.2026; blocked by 4.48.1)]`
...
### 4.56. Визуальная стилистика web: фоны локаций `[L / M / todo (M → L 21.05.2026; обсуждение 12.05.2026)]`
...
### 4.57. Equipment characteristic `'energy_regen'` (V2 для 4.21) `[L / M / todo (M → L 21.05.2026; blocked by 4.21)]`
...
### 4.58. Active buffs system (state + display + tick + integration) `[L / M / todo (M → L 21.05.2026 — текущий дизайн игры не подразумевает active skills; нужно решить нужны ли; pre-requisite для 4.7.2)]`

**⚠️ Design discussion (21.05.2026):** Текущая механика игры — **cozy passive system**: все skills (Gym + char_level + equipment) — пассивные multipliers, начисляются автоматически. **Active skills с коротким cooldown** (например +50% Speed на 30 мин, cooldown 4 часа) — **противоречат cozy nature**: cooldown management = work, не fun; iPhone-unfriendly (нужны notifications); ломает async-flow «открыл когда удобно».

**Решение по дизайну active skills:**

1. **Когда добавлять — только long-cooldown (23+ часа).** На коротких cooldown'ах появляется давление «не забыть активировать» → cozy ломается. На длинных (daily / weekly) воспринимается как «daily reward», не как ability management.
2. **Бонус должен быть нужен прямо сейчас.** Active skill даёт не abstract multiplier, а конкретное действие в текущей ситуации (восполнить ресурс, разблокировать что-то, спасти streak). Игрок принимает решение «сейчас или подождать».
3. **Триггер через achievement.** Active skills открываются за достижение через Triumphs (4.62), а не доступны с начала игры. Это feels как награда, не как obligation.

**Идея active skill (для 4.62 / Triumphs intergration):**

- **🔥 Streak Saver** — «Продолжить дневной streak». Если игрок пропустил **один** день (e.g. yesterday — 0 шагов или <10k для daily_bonus), активация скилла восстанавливает streak (заполняет вчера как «качественный»). Cooldown — **7 дней**.
- **Unlock:** триумф за 1,000,000 потраченных шагов (точное значение — на дизайн в 4.62).
- **Почему 7 дней:** достаточно длинный чтобы не использовать каждый «случайный» пропуск, достаточно короткий чтобы быть полезным real life (отпуск / болезнь / forgotten phone).
- **Применение:** один тап в CLI / web — confirm prompt «Восстановить streak за {yesterday_date}?» → daily_bonus сохраняется → log_event('streak_saved').

**Pre-requisite для 4.7.2** (бустеры в Shop) — там shop boosters это **item-based active mechanic** (купил → выпил когда нужно), что лучше cozy fit'ит чем skill-based active. Implementation infra (state.active_buffs / expire_old_buffs / tick) общая.
...
### 4.59. Кузница / Blacksmith (зонтичная — Repair + Crafting + Gems) `[M / L / partial — всё закрыто КРОМЕ 4.59.3 Gems (deferred low-priority): 4.59.0 infra + 4.59.1 Repair + 4.59.2 Crafting (0.2.4o) + 4.60 навыки/gate (0.2.6a) + 4.48.11 web (0.2.6c) + 4.59.4 таймеры CLI+web (0.2.6e)]`
...
### 4.62. Triumphs / Достижения (зонтичная — объединяет 4.4 + 4.44) `[H / L+ / partial — core done (0.2.5j..0.2.6): Phase 1 foundation + Phase 2 catalog 5 категорий (40 triumph'ов) + Phase 3 meta (seals/titles/pinned/claim) + Phase 5 web. Остаток: Phase 2 extended catalog (4.62.1.7-14), Phase 4 bonuses (deferred), Phase 6 hidden (deferred)]`
...
- **4.58 Active buffs** (todo) — нужна для временных bonus-эффектов на части триумфов. Блокирует 4.62.2 для **временных** бонусов, но не для постоянных. **Активные skills как rewards триумфов** — см. design discussion в 4.58 (21.05.2026): long-cooldown skills (23+ часа) которые открываются achievement'ами, дают «бонус нужный прямо сейчас» (не abstract multiplier). Пример: **Streak Saver** (cooldown 7 дней) разблокируется за 1,000,000 потраченных шагов, восстанавливает daily_bonus streak при пропуске одного дня.
...
- ✅ **Crafter (Total items crafted)** — done 28.05.2026 (0.2.6f). Counter, hook `item_crafted`, `count_delta=+1` (крафт даёт 1 предмет). Tiers **[5, 10, 25, 50, 100]**. Backfill авто (item_crafted уже в history). **Без seal** у категории forge (решено 28.05.2026). Каталог 52→53. Тесты: 6 (`TestCrafter`).
- ❌ **First S+ item crafted** — **ОТМЕНЕНО (28.05.2026)**. Решили не делать: one-shot мало ценности + потенциальный нахлёст с 4.62.1.14 Collection «Max grade — first S+ owned» (любой источник). Если понадобится — там и учтём.
...
**🔋 Energy (split by source / total):**
- **Total energy spent** — 10k / 50k / 200k / 1M потраченной энергии (всех типов). Bonus capstone: **+5 / +10 max energy** или **+2 / +5 к скорости регенерации** (`bonus.energy_regen_interval`). **Баланс TBD:** +5 = ~8% от current 65, +10 = ~15% — заметно но не game-breaking; помогает chicken-and-egg progression к Stamina lvl 19+ (cost 95 эн).
- **Energy per Adventure** — отдельный триумф за энергию потраченную на приключения (использует existing `apply_energy_optimization_adventure` chain). Тиерированный.
- **Energy per Training** — то же для gym training.
- **Energy per Work** — то же для work shifts.
- *Why split:* мотивирует diverse play. Игрок-«работяга» получит свой триумф, игрок-«авантюрист» — свой.

**🏭 Work (general + per-vacancy):**
- **Total work hours** — 10 / 100 / 1000 / 10000 часов (любая работа). Bonus: **+5% к ЗП** (постоянный, stack с earnings_boost 4.23) или **-2% к времени смены** (постоянный, stack со speed_skill).
- **Per-vacancy hours** — отдельные триумфы для каждой работы: Watchman / Factory / Courier_foot / Forwarder. Тиеры 10/50/100/500. Bonus capstone — vacancy-specific: например Watchman → +1 к stamina, Factory → +1 к energy_max, Courier → +1 к speed, Forwarder → +1 к luck (характеристики связаны со steps spent on this vacancy).
- *Why split:* поощряет разнообразие — игрок попробует все вакансии, не зацикливаясь на одной.

**🗺 Adventure (general + per-adventure):**
- **Total adventures** — уже есть в MVP (10/50/100/500/1000).
- **Per-adventure completions** — отдельные триумфы для каждой прогулки (walk_easy / normal / hard / 15k / 20k / 25k / 30k). Тиеры 10/50/200/1000. Bonus capstone: +N% drop rate (только для этой adventure) или unlock special drop variant.
- *Why split:* поощряет ходить разные walks, не только endgame (walk_30k).

**💰 Money spent on skills:**
- **Total money spent on Gym** — 1k / 10k / 100k / 1M $. Bonus capstone: **+5% к money_saving** (постоянный stack с 4.20).
- *Idea:* recognizes деньги-invest в long-term progression.

**🔥 Streak / consistency:**
- **Daily streak record** — longest consecutive days с 10k+ шагов (daily_bonus earned). Тиеры 7 / 30 / 100 / 365 дней. Bonus capstone: +N% к daily_bonus.
- **Total days played** — 7 / 30 / 100 / 365 / 1000 / 3650 (10 лет!) уникальных дней (НЕ обязательно подряд). **Tracking:** счётчик на каждый день в котором было хотя бы одно ввод шагов (>0 entries в `steps_log` за этот день) — наиболее надёжный сигнал «игрок активен». Альтернатива: дата в `state.date_last_enter` каждый раз когда меняется → инкремент counter. Bonus capstone (например 1000 дней): постоянный +5% к чему-то значимому, либо unlock эксклюзивного title «Loyal» / «Veteran». Trophy-feel для long-term engagement.

**🏦 Bank:**
- **Total deposit interest earned** — общая сумма capitalized процентов. Bonus: +N% к banking_interest_rate.
- **Total loans repaid** — за количество закрытых кредитов. Trophy.
- **Max single deposit** — биггест deposit amount в один момент. Trophy.

**🔨 Forge:**
- **Total repairs done** — счётчик ремонтов. Bonus: +N% к neatness_in_using_things.
- **Total items crafted** — счётчик crafts. Trophy.
- **First S+ item crafted** — milestone.

**🎒 Inventory / Collection:**
- **Unique item_type × characteristic combinations collected** — за разнообразие inventory. Trophy.
- **Max item quality reached** — first item quality 90+ / 100. Trophy.
- **Max grade collected** — first S+ item ever found. Trophy.

**⭐ Level / progression:**
- **Char_level milestones** — 5 / 10 / 25 / 50 / 100 (уже в MVP).
- **Skill mastery (per-skill)** — отдельный триумф за прокачку каждого Gym-навыка до 30 / 50 / 100. Bonus capstone: per-skill (specific to skill).
- **Total skills levels** — общий счётчик `sum(state.gym.*)` по всем Gym-навыкам (20 skills × N levels). Тиеры: 50 / 200 / 500 / 1000 / 5000 total levels. Bonus capstone: например +1 ко всем passive bonuses или unlock title «Master of All Trades». Trophy-feel за **широкое** развитие vs «вылить всё в один skill».
- **Skill diversity** — bonus за прокачку **всех** 20 Gym-навыков хотя бы до уровня N (5 / 10). Поощряет балансированную прокачку, не «only-stamina» билд.

**🕒 Time-of-day / lifestyle:**
- **Early walker** — 10k+ шагов до noon в N дней.
- **Evening walker** — same для evening.
- **Weekend warrior** — 50k+ шагов за выходные.
- Mostly trophy, не gameplay bonus (lifestyle stats).

**Active skill rewards (через 4.58 — см. 4.58 design discussion):**
- **🔥 Streak Saver** — за 1M потраченных шагов. Cooldown 7 дней. Восстанавливает daily_bonus streak.
- **🎁 Lucky Day** — за 10 S-grade дропов. Активный, cooldown 24h. +50% Luck на следующее приключение.
- **💸 Premium Shift** — за 1000 рабочих часов. Cooldown 24h. +50% ЗП на следующую смену.
- *Все active skills — long cooldown (24h+) и дают «бонус нужный прямо сейчас», не abstract multiplier* (см. дизайн в 4.58).

**Balance discussion items** (TBD при реализации):
1. **Max energy +5/+10** — не overpowered? Считаем: +10 = ~15% от current 65 → meaningful, но не game-breaking. Дополнительный stat boost к existing energy_max_skill / equipment / level.
2. **Per-vacancy stat bonuses** — может cause «обязательная прокачка всех вакансий» если каждая даёт уникальный stat. Балансировать через capstones tier (только за 500+ часов).
3. **Не дублировать с Gym skills** — например триумф «+5% money_saving» дублирует Gym skill. Должны давать НОВЫЙ stat / unlock, не stack с тем что качается в Gym? Или explicit stack? Дизайн при реализации.
4. **Total counts** — за 10k энергии триумф S-tier (+10 max energy) — может стать слишком easy unlock'ом для long-time игрока. Адаптировать тиеры под скорость игры (currently ~1.8M total used steps за 6 месяцев → 200k energy spent очень mid-game).

**Что я добавил от себя (вне user's brainstorm):**
- Streak / consistency category (longest streak, days played — long-term loyalty triumph до 10 лет)
- Bank category (interest earned, loans repaid)
- Forge category (repairs, crafts)
- Collection / Inventory category (unique combos, max quality)
- Skill mastery per-skill (для каждого Gym skill до 30/50/100)
- Time-of-day lifestyle triumphs
- Active skill rewards (Lucky Day, Premium Shift) для дополнения Streak Saver

**Что пользователь предложил:**
- Energy spent total + split (adventure/training/work)
- Energy regen bonus как capstone
- Work hours general + per-vacancy
- Adventure general + per-adventure
- Money spent on skill leveling
- **Total days played** (21.05.2026) — не обязательно подряд, tracking через steps_log entries за день
- **Total skills levels** (21.05.2026) — общий sum по всем Gym-skills, поощряет широкое развитие
...
**Финализированный дизайн (29.05.2026):**

| Решение | Выбор |
|---|---|
| Где | Только `drop.item_type()` (выбор ТИПА). Grade-гейт «выпадет ли что-то» НЕ трогаем — меняем только распределение типов. |
| Механизм | `item_type()` переходит с «5 кубиков max» на **взвешенную выборку** (`random.choices(types, weights)`) — иначе «18%+luck%» не выразить. Попутно уходит quirk «ничья кубиков → None» (≈2.5% подтверждённых дропов сгорало). Пре-гейт рюкзака 5% (4.51) встраивается как базовый вес. |
| База весов | backpack 5, остальные 5 типов по 19 (≈ текущее распределение). |
| «Нужность» слота | По ГРЕЙДУ (не quality). Тиры: empty=0, C=1, B=2, A=3, S=4, S+=5. Для типа берём **лучший в наличии** (equipment + inventory); `gap = 5 − best_owned_tier`. |
| **Кольца (2 слота)** | **Вариант R2:** gap по **худшему из двух НАДЕТЫХ пальцев** (пустой палец = worst). Стимул заполнить оба finger-слота. (Отличается от single-slot basis «лучший в наличии» — намеренно, т.к. 2 слота.) |
| Формула буста | **Относительная** (худший слот → ПОЛНЫЙ luck): `max_gap = max(gap по всем типам)`; `boost_type = luck × gap_type / max_gap`; `weight = base + boost`; ренорм. Если `max_gap=0` (всё S+) → буста нет. |
| luck | **Сырой** `current_luck` (без cap) — его для этого и качают. Cap — опционально позже если перекос. |
| Кривая gap→frac | **Линейная** по тирам (через max_gap нормировку). |

**Почему относительная (а не абсолютная дистанция):** точно отражает оба сценария игрока — (а) пустой слот + A-grade: пустой = худший → полный luck, A → часть; (б) A-grade единственный не-S+: A = худший → полный luck. Абсолютная дала бы A всегда ~0.4·luck и в (б) не совпала бы с «полный буст».

**Примеры (luck=30, база backpack 5 / типы 19):**
- *Нет рюкзака, шлем A, остальное S+:* backpack gap5 → +30 (вес 35, **24.6%**), helmet gap2 → +12 (вес 31, 21.8%), S+ типы 19 (13.4% каждый).
- *Шлем A единственный не-S+:* helmet gap2, max_gap=2 → +30 (вес 49, **37.7%**), S+ типы 14.6%, backpack 3.8%.

**Реализация (эскиз):** `drop.item_type()` → построить dict весов (base + boost по формуле), `random.choices`. Helper `_type_gap(state, item_type)` (best-owned для single-slot; worst-of-2-equipped для ring). `current_luck` уже есть. Тесты: gap по грейду/empty, ring R2, относительный буст (worst→full), max_gap=0 → base, MC-проверка итоговых % на примерах.

**Зависимость:** усиливает 4.51, но не блокирует. Пересмотреть вместе с 4.19. NB: type-boost не зависит от грейда, доступного в текущем adventure (всегда дистанция от S+) — сознательное упрощение (решено 29.05.2026).

---

### 4.66. Мульти-параметрные предметы (несколько характеристик) `[L / L / todo (отложено, 29.05.2026 — выделено из 4.51)]`

**⚠️ Большая отложенная задача (low priority).** Сейчас предмет несёт **ровно одну** характеристику: item-dict `characteristic: [одно]` + `bonus: [одно]`, `equipment_bonus` читает `[0]`. Эта задача — позволить предмету иметь **несколько** статов одновременно (напр. рюкзак: +6 слотов И +N stamina; или кольцо +luck +energy).

**Скоуп (большой — правит весь item-стек):**
- Item-модель: `characteristic`/`bonus` → список пар (или dict `{char: bonus}`); миграция legacy (одна пара).
- `equipment_bonus` агрегаторы (5 функций) — суммировать по всем характеристикам предмета.
- `drop.py` — генерация предметов с 1..N характеристиками (вероятность доп-стата?).
- `forge` crafting — как комбинировать мульти-стат предметы (avg по каждой? identity-ключ?).
- Loadout-оптимизатор (4.63) — предмет ценен по нескольким осям, current greedy-by-one-char ломается.
- Display (CLI + web) — показывать все статы.
- TypedDict / 1.6 (Items as dataclass, rejected) — возможно пересмотреть.

**Разблокирует:** «рюкзак + stamina» (4.51 V2), богатые предметы вообще. **Зависимость:** независима, но трогает много кода — делать осознанно, когда захотим глубины в экипировке.

---

### 4.67. Streak decay — мягкое затухание стрика вместо обнуления `[M / M / todo (нужно решить модель + баланс, 17.06.2026)]`

**Идея (обсуждение 17.06.2026):** сейчас `daily_bonus` (стрик дней с 10k+ шагов) при пропуске **жёстко сбрасывается в 0** — один пропущенный день стирает недели прогресса. Хочется смягчить: при пропуске **не обнулять, а уменьшать** стрик. Снимает фрустрацию, лучше ложится на cozy-тон игры. Классическая боль, решаемая в индустрии (Duolingo Streak Freeze, Habitica HP-decay и т.п.).

**Текущая механика (точка правки — одна строка):** `functions.py:today_steps_to_yesterday_steps` на смене дня: вчера ≥ 10k → `daily_bonus += 1`, иначе `daily_bonus = 0`. `daily_bonus` даёт **двойной бонус** — `+daily_bonus %` к шагам И `+daily_bonus` к макс. энергии (`functions.py:265-266`, `compute_energy_max`), **без cap'а**. Отдельно `daily_streak_record` (monotonic max) питает триумф **On Fire** (4.62.1.9 — «макс дней ПОДРЯД с 10k+»).

**Рассмотренные модели штрафа за пропуск** (поведение на примере стрика 30 — пропусков до обнуления):

| Модель | Формула | Стрик 30 | Характер |
|---|---|---|---|
| Жёсткий сброс (сейчас) | `= 0` | 1 | строго |
| Линейный −1 | `-= 1` | ~30 | очень липко, нужен cap |
| **−⅓ пропорц.** | `= floor(s·0.67)` | ~8 | мягко, самомасштабируется |
| **−½ пропорц.** | `= floor(s/2)` | ~5 | сбалансированно, scale-invariant |
| **Ускоряющийся** | −1,−2,−4… за каждый пропуск подряд (reset при возврате) | ~5 (1-й день всего −1) | «умный»: щадит разовый срыв, рушит заброс |
| Grace + сброс | N бесплатных пропусков, потом 0 | — | = Streak Freeze (4.36) |
| Чекпоинты-полы | падает до последнего «замка» (кратно 7/10) | 30→20 | геймно |

**Предварительный фаворит (НЕ решено):** пропорциональный **−½** (простой, честный, самоограничивающийся) либо **ускоряющийся** (лучшая семантика «прости разовый срыв, накажи заброс»). Финальный выбор + числа — после балансового прохода (возможно мини-симулятор как для pity 4.19).

**Что нужно решить / учесть (для ЛЮБОЙ не-жёсткой модели):**
1. **Cap на `daily_bonus`.** Сейчас безлимит + двойной эффект — при «не-обнулении» большой бонус становится липким. Пропорц./ускоряющийся гасят это сами, линейный −1 — нет. Решить потолок (напр. 50/100) или оставить self-limiting.
2. **Отдельный consecutive-счётчик для On Fire.** Любая модель кроме жёсткого сброса ломает «дней ПОДРЯД» → завести настоящий consecutive-counter именно для триумфа, а `daily_bonus` сделать «momentum». Правка в 4.62.1.9 / `daily_streak_record`.
3. **Округление / пол 0** (floor, не уходить ниже 0).

**Связь с другими задачами:** дополняет (не заменяет) **4.36 Streak Freeze** — freeze просто пропускает применение штрафа в защищённый день. Альтернатива отклонённой **4.35** (Exponential Daily Bonus). Трогает триумф **On Fire** (4.62.1.9).
...
#### 4.68.1. Стрик в Stats (CLI + web) `[M / S / done (17.07.2026, 0.2.7i)]`

**Проблема:** daily-бонус невидим — вшит в тоталы Steps/Energy, детали в свёрнутых «📈 Бонусах»; момент same-day активации (4.68) проходил незамеченным. Единственная механика с ежедневным «использовать или потерять» не работала как мотиватор.

**Реализовано (финальный дизайн после live-итерации):** web Stats — компактная `🔥 Streak: N day(s)` ПОСЛЕ Level, только при активном стрике (`days > 0`), без деталей; вся детализация («до бонуса X шагов» / «N дн ✓ +N% 🏃 · +N 🔋») — новая строка в «📈 Бонусах». CLI — детальная строка между Energy и Money в status_bar (места хватает). N = `effective_daily_bonus` в обоих состояниях (до порога равен хранимому стрику). `streak_view` = {days, active, remaining} в context. Реализует идею отклонённой 4.3 («🔥 N дней подряд» как мотиватор) без нового поля. 5 тестов (CLI оба состояния; web: компакт при активном / скрыт при нулевом / детали в Бонусах). 1556 passed.
