...
### 4.65. Drop probability modifier: нет предмета → +luck к шансу `[M / M / todo (29.05.2026 — выделено из 4.51)]`

**⚠️ Идея, требует дизайна.** Soft-pity / completion-incentive. Связана с **4.19 (Pity system)** — возможно объединить.

**Суть:** если предмета (тип / рюкзак) у игрока **нет** в inventory+equipment — повысить вес его выпадения в ролле дропа на `current_luck(state)`. Стимул добивать недостающие типы; естественно дружит с рюкзаком (4.51) — пока рюкзака нет, его 5%-шанс растёт.

**Где:** `drop.item_type()` (шаг выбора типа, НЕ grade-гейт). Сейчас 5 независимых `randint(1,100+luck)`, max побеждает + пре-гейт рюкзака (4.51). Модификатор: для отсутствующих у игрока типов — поднять верхнюю границу ролла / вес на `+luck` (или буст пре-гейта рюкзака). **НЕ трогать общий шанс «выпадет ли что-то»** — только распределение типов.

**Открытые вопросы:** какая именно формула буста (аддитив к границе ролла vs множитель веса); считать «нет предмета» = нет в inventory+equipment, или учитывать историю владения (никогда не находил)?; применять ко всем типам или только к «целевым» (рюкзак / отсутствующие слоты). Согласовать при реализации.

**Зависимость:** усиливает 4.51, но не блокирует её (4.51 шипит флэт-5%). Пересмотреть вместе с 4.19.
