...
### 4.35. Exponential Daily Bonus + Streak Counter `[H / M / todo]`

Текущая логика: при ≥10k шагов вчера → `steps_daily_bonus += 1`, иначе сброс. Бонус линейный.

**Идея:** многоуровневые тиры по дням стрика. Сильнее награждает долгую дисциплину.

| Дни стрика | Множитель к Daily Bonus |
|---|---|
| 1 | +5% |
| 7 | +25% |
| 30 | +100% |
...
Точные числа подобрать через симулятор экономики.

**Связь с 4.3 (Streak Counter):** эта задача поглощает 4.3 — добавляется поле `streak_days` в `char_characteristic`, обновляется ежедневно.

**Демотивация при сбросе** — острая проблема (пользователь сам подтвердил). Решение — Streak Freeze (см. **4.36**).

**Реализация:**
1. Поле `streak_days` в `char_characteristic`.
2. В `today_steps_to_yesterday_steps()`: если ≥10k → `streak_days += 1`, иначе → `streak_days = 0` (с учётом Streak Freeze).
3. Функция `streak_bonus_multiplier()` — табличный lookup по тирам.
4. UI: "🔥 Стрик: 12 дней / +30%" в `status_bar()`.
5. Save в трёх форматах.

**Зависимость:** связана с 4.36 (Streak Freeze).
