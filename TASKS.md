...
### 4.7. Shop (зонтичная — framework + каталог items) `[M / M / todo (объединена с 4.40 — 14.05.2026)]`
...
### 4.54. Sync resolution: CLI ↔ Web `[H / M / todo (приоритет повышен M→H 15.05.2026 — блокирует 4.48.0.1 deploy)]`
...
**Эскалация (15.05.2026):** при попытке deploy web на постоянно работающий сервер (4.48.0.1) — становится реальным блокером. Сервер крутит web 24/7, любой запуск CLI на ноутбуке создаёт окно tihogo overwrite'а (CLI с stale RAM-snapshot → `s`/`q` затирает прогресс с сервера). До решения 4.54 deploy невозможен без жёсткого правила «CLI больше не запускаем».

**Возможные стратегии (для будущей дизайн-сессии — архитектурные решения пока НЕ приняты):**

- **Optimistic concurrency (last_modified timestamp)** — один timestamp в Sheets, при save pre-check «не моя ли версия устарела», на конфликте отказ с предложением reload. Простой (S/M), atomic «всё-или-ничего», без merge.
- **Per-field timestamps + merge** — каждое поле имеет свой `_updated_at`, на save merge по полю newer-wins. Сложный (M/L), позволяет real concurrent игру, но требует решений по nested fields (inventory list, equipment dict, и т.д.).
- **Event sourcing** — append-only log событий (есть полу-готовая инфра через `history.jsonl` + 24 типа events). State = `replay(events)`. Большой (L), но conflict-free.
- **Lock-механизм** — cell в Sheets с `session_active_since` / `session_token`. Простой, но прост в deadlock-сценариях.
- **Auto-save в CLI на каждом действии** — снизит окно расхождения, не решает problemу в корне.
- **Detection с warning** — base layer для любой стратегии: "Web session active 2 min ago — данные могут быть stale".

**Не делаем сейчас:** архитектурный выбор стратегии. Решение принимаем отдельной дизайн-сессией перед реализацией.

**Effort:** **M** (по стратегии optimistic timestamp) до **L** (event sourcing).
