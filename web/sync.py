...
def persist_state_to_cloud() -> Literal["OK", "STALE"]:
    """Save state (CSV + Sheets save_safe) с optimistic concurrency.

    4.54.4 — теперь возвращает status. На "STALE" caller (mutation endpoints
    в web/main.py, 4.54.6) показывает flash toast + auto-reload.

    Returns:
    - **"OK"** — save прошёл (или Sheets unavailable — CSV-only fallback).
    - **"STALE"** — Sheets имеет newer state, save aborted. Web endpoint
      возвращает 200 со специальным flash-fragment'ом + auto-reload через 2с.

    Pre-4.54: возвращал None и сейв в Sheets был отдельным `GameStateRepo().save()`
    после `save_characteristic()`. Теперь обе операции консолидированы в
    `save_characteristic()` через `save_safe()` — он сам делает CSV+Sheets.

    Применяется после каждой мутирующей web-операции (start/add_hours смены,
    day rollover, новые шаги). Жил исторически в web/main.py, перенесён сюда
    в 0.2.1b чтобы try_reload_state мог зайти после rollover-а без
    циркулярного импорта (web/sync ↔ web/main).
    """
    return save_characteristic()
